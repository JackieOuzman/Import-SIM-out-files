...
Condobolin <- read.csv(paste0("X:/Summer_weeds/APSIM_7/NSW_sites/output_csv/", site, "extra_Clm_step2.csv"))
Condobolin <- Condobolin %>% mutate(site = "Condobolin")

site <- "Cootamundra_top_up"
Cootamundra <- read.csv(paste0("X:/Summer_weeds/APSIM_7/NSW_sites/output_csv/", site, "extra_Clm_step2.csv"))%>% mutate(site = "Cootamundra")

site <- "Merriwagga_top_up"
Merriwagga <- read.csv(paste0("X:/Summer_weeds/APSIM_7/NSW_sites/output_csv/", site, "extra_Clm_step2.csv"))%>% mutate(site = "Merriwagga")

df <- rbind(Condobolin, Cootamundra, Merriwagga)
rm(Condobolin, Cootamundra, Merriwagga)
##################################################################################
...
ggsave(
  device = "png",
  filename = "Cootamundra_Biomass.png",
...
df_weeds_summary %>% 
  filter(site == "Condobolin") %>% 
  ggplot( aes(x=as.factor(initial_water), y = Av_weed_biomass, colour = as.factor(weed_sow_and_emergedate_month)))+
  geom_point()+
  ylim(Biomass_min, Biomass_max)+
  facet_grid(grouping~soil)+
  theme_bw()+
  labs(title = "Condobolin",
       #subtitle = "Weed type  = summer_grass,  \nweed density = 12, weed kill = 200",
       x= "inital water", y = "weed biomass kg/ha",
       colour = "Weed window")+
  theme(
...
ggsave(
  device = "png",
  filename = "Condobolin_Biomass.png",
  path= "X:/Summer_weeds/APSIM_7/NSW_sites/plots/",
  width=8.62,
  height = 6.28,
  dpi=600
...
df_weeds_summary %>% 
  filter(site == "Merriwagga") %>% 
  ggplot( aes(x=as.factor(initial_water), y = Av_weed_biomass, colour = as.factor(weed_sow_and_emergedate_month)))+
  geom_point()+
  ylim(Biomass_min, Biomass_max)+
  facet_grid(grouping~soil)+
  theme_bw()+
  labs(title = "Merriwagga",
       #subtitle = "Weed type  = summer_grass,  \nweed density = 12, weed kill = 200",
       x= "inital water", y = "weed biomass kg/ha",
       colour = "Weed window")+
  theme(
...
ggsave(
  device = "png",
  filename = "Merriwagga_Biomass.png",
  path= "X:/Summer_weeds/APSIM_7/NSW_sites/plots/",
  width=8.62,
  height = 6.28,
  dpi=600
