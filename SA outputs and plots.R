...
## import data
site <- "Minnipa_top_up"
Minnipa <- read.csv(paste0("X:/Summer_weeds/APSIM_7/SA_sites/output_csv/", site, "extra_Clm_step2.csv"))
Minnipa <- Minnipa %>% mutate(site = "Minnipa")

site <- "Loxton_top_up"
Loxton <- read.csv(paste0("X:/Summer_weeds/APSIM_7/SA_sites/output_csv/", site, "extra_Clm_step2.csv"))%>% mutate(site = "Loxton")

site <- "Roseworthy_top_up"
Roseworthy <- read.csv(paste0("X:/Summer_weeds/APSIM_7/SA_sites/output_csv/", site, "extra_Clm_step2.csv"))%>% mutate(site = "Roseworthy")

df <- rbind(Minnipa, Loxton, Roseworthy)
rm(Minnipa, Loxton, Roseworthy)
##################################################################################
...
df_weeds_summary %>% 
  filter(site == "Roseworthy") %>% 
  ggplot( aes(x=as.factor(initial_water), y = Av_weed_biomass, colour = as.factor(weed_sow_and_emergedate_month)))+
  geom_point()+
  ylim(Biomass_min, Biomass_max)+
  facet_grid(grouping~soil)+
  theme_bw()+
  labs(title = "Roseworthy",
       #subtitle = "Weed type  = summer_grass,  \nweed density = 12, weed kill = 200",
       x= "inital water", y = "weed biomass kg/ha",
       colour = "Weed window")+
  theme(
...
ggsave(
  device = "png",
  filename = "Roseworthy_Biomass.png",
  path= "X:/Summer_weeds/APSIM_7/SA_sites/plots/",
  width=8.62,
  height = 6.28,
  dpi=600
...
yld_diff_df <- yld_diff %>% mutate(Yld_diff =(Av_weed_yield_controlled -Av_weed_yield_uncontrolled)/1000) 
